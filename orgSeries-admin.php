...
function orgSeries_admin_script()
{
    ...
    if (('edit-tags.php' == $pagenow || 'term.php' == $pagenow) && ppseries_get_series_slug() == $taxonomy) {
...
function orgSeries_admin_assets()
{
    ...
    if (is_ppseries_admin_pages()) {
        ...
        $org_opt = get_option('org_series_options', []);
        $default_post_list_box_selection = 0;
        if (class_exists('PPS_Post_List_Box_Utilities')) {
            $default_post_list_box_selection = (int) PPS_Post_List_Box_Utilities::get_default_post_list_box_id();
        }
        wp_localize_script('pps-admin-js', 'ppseriesSettings', [
            ...
            'saved'   => [
                ...
                'series_post_navigation_selection' => isset($org_opt['series_post_navigation_selection']) ? (int) $org_opt['series_post_navigation_selection'] : 0,
            ],
        ]);
    }
}
...
function admin_ajax_series()
{
    ...
    $name = isset($_POST['newseries']) ? sanitize_text_field(wp_unslash($_POST['newseries'])) : '';
...
function admin_inline_series_ajax()
{
    ...
    $data[$seriesid] = isset($_POST['post_series']) ? sanitize_text_field(wp_unslash($_POST['post_series'])) : '';
    $data[$series_part] = isset($_POST['series_part']) ? sanitize_text_field(wp_unslash($_POST['series_part'])) : '';
    $data[$id] = isset($_POST['series_post_id']) ? sanitize_text_field(wp_unslash($_POST['series_post_id'])) : '';
    $data[$is_series_save] = isset($_POST['is_series_save']) ? sanitize_text_field(wp_unslash($_POST['is_series_save'])) : 0;
...
function get_series_list($default = 0)
{
    ...
    if (isset($post) && is_object($post)) {
        $post_ID = $post->ID;
    } elseif (isset($postdata) && is_object($postdata)) {
        $post_ID = $post->ID;
    } else {
        $post_ID = false;
    }
...
function orgSeries_custom_column_filter($defaults)
{
    ...
    $request_post_type = isset($_REQUEST['post_type']) ? sanitize_key(wp_unslash($_REQUEST['post_type'])) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only posts-list column detection.
    if (!empty($request_post_type) && !in_array($request_post_type, $post_types)) {
...
function orgSeries_custom_manage_posts_filter()
{
    ...
    $series_name = '';
    if (isset($_GET[ SERIES_QUERYVAR ])) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only posts-list filter state.
        $series_name = sanitize_text_field(wp_unslash($_GET[ SERIES_QUERYVAR ])); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only posts-list filter state.
    }
