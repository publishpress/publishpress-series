...
function orgSeries_admin_script()
{
    ...
    if (('edit-tags.php' == $pagenow || 'term.php' == $pagenow) && isset($_GET['taxonomy']) && ppseries_get_series_slug() == $_GET['taxonomy']) {
...
function admin_ajax_series()
{
    ...
    $name = (isset($_POST['newseries']) && is_string($_POST['newseries']) ? sanitize_text_field(wp_unslash($_POST['newseries'])) : '');
...
function orgSeries_custom_manage_posts_filter()
{
    ...
    if (isset($_GET[ SERIES_QUERYVAR ])) {
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only screen selection or display filter; no request-driven mutation.
        $series_name = sanitize_text_field(wp_unslash($_GET[ SERIES_QUERYVAR ]));
    }
