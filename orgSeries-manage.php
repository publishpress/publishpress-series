...
$request_taxonomy = ! empty($_REQUEST['taxonomy']) ? sanitize_key(wp_unslash($_REQUEST['taxonomy'])) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only taxonomy screen detection.
if (! empty($request_taxonomy) && in_array($request_taxonomy, [ppseries_get_series_slug(), 'series_group'], true)) {
...
function wp_insert_series($series_id, $taxonomy_id)
{
    ...
    $series_icon = isset($_POST['series_icon_loc']) ? esc_url_raw(wp_unslash($_POST['series_icon_loc'])) : null;
...
{
    global $_POST, $wpdb;
    $series_icon_loc = isset($_POST['series_icon_loc']) ? esc_url_raw(wp_unslash($_POST['series_icon_loc'])) : '';
    $delete_image = isset($_POST['delete_image']) ? sanitize_text_field(wp_unslash($_POST['delete_image'])) : false;
    if (empty($series_icon_loc)) {
        $series_icon_loc = '';
    }
...
function ajax_pp_series_reordering_terms()
{
    ...
    $terms = map_deep(wp_unslash($_POST['terms']), 'sanitize_text_field');
    $nonce = sanitize_text_field(wp_unslash($_POST['nonce']));
    $tax = sanitize_key(wp_unslash($_POST['tax']));
...
function pp_series_terms_clauses($clauses, $taxonomies, $args)
{
    ...
    if (is_admin()) {
        ...
        $orderby = isset($_GET['orderby']) ? sanitize_key(wp_unslash($_GET['orderby'])) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only term list ordering state.
        if (!empty($orderby) && $orderby !=  'term_order') {
            return $clauses;
        }
