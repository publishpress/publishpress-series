...
if ($delete_grouping == 1) {
    ...
    $query = $wpdb->prepare("SELECT ID from $wpdb->posts WHERE post_type = %s", $type);
    $series_grouping_posts = $wpdb->get_results($query); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Prepared immediately above.
