...
function get_series_groups($args = array())
{
    ...
    if (!empty($exclude_tree)) {
        ...
        foreach ($excluded_trunks as $extrunk) {
            $excluded_children = (array) get_terms(array('taxonomy' => 'series_group', 'child_of' => intval($extrunk), 'fields' => 'ids'));
    ...
    // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Fragments use fixed columns, integer IDs/limits and an allowlisted direction; taxonomy is prepared.
    ...
    if ('count' == $fields) {
        $group_count = $wpdb->get_var($query); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Query prepared above.
        return $group_count;
    }

    $series_groups = $wpdb->get_results($query); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Query prepared above.
    ...
    if (0 < $number && count((array) $series_groups) > $number) {
