...
function orgseries_grouping_upgrade($this_version, $old_version)
{
    ...
    if ($old_version == '1.6') {
        ...
        $query_args = array_merge($groups, array('series_group'));
        $query = $wpdb->prepare(
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- IN placeholders are generated from integer group IDs above.
            "SELECT term_taxonomy_id FROM $wpdb->term_taxonomy WHERE term_id IN ( $group_placeholders ) AND taxonomy = %s",
            $query_args
        );
        $terms = $wpdb->get_results($query); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Prepared above with integer placeholders.
...
function upgrade_orgseries_grouping_from_one_five()
{
    ...
    $_old_groups = get_terms($args_b); //need to do this in order to get the description field.
...
// phpcs:disable WordPress.Security.NonceVerification.Recommended -- These callbacks only read sanitized admin filter/view state and do not mutate data.
...
{
    global $_GET;
    $series_group = isset($_GET['ser_grp']) ? absint(wp_unslash($_GET['ser_grp'])) : 0;
    if (!empty($series_group) && is_admin()) {
...
{
    global $_GET;
    if (in_array(ppseries_get_series_slug(), $taxonomies) && isset($_GET['ser_grp'])) {
        $group_id = absint(wp_unslash($_GET['ser_grp']));
...
function orgseries_groups_scripts()
{
    ...
    if ('edit-tags.php' == $checkpage && ppseries_get_series_slug() == $taxonomy) {
...
function orgseries_groups_styles()
{
    ...
    if ('edit-tags.php' == $checkpage && ppseries_get_series_slug() == $taxonomy && 'edit' == $action) {
...
// phpcs:disable WordPress.Security.NonceVerification.Missing -- WordPress verifies the term add/edit nonce before created_/edited_ taxonomy hooks fire.
function wp_insert_series_group($series_id, $taxonomy_id)
{
    global $_POST;
    $tax_input = isset($_POST['tax_input']) && is_array($_POST['tax_input']) ? map_deep(wp_unslash($_POST['tax_input']), 'sanitize_text_field') : array();
