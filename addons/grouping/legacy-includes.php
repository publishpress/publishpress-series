...
function orgseries_grouping_upgrade($this_version, $old_version)
{
    ...
    if ($old_version == '1.6') {
        ...
        $group_placeholders = implode(', ', array_fill(0, count($groups), '%d'));
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- The IN fragment contains only generated integer placeholders.
        $query = $wpdb->prepare("SELECT term_taxonomy_id FROM $wpdb->term_taxonomy WHERE term_id IN ( $group_placeholders ) AND taxonomy = 'series_group'", $groups);
        $terms = $wpdb->get_results($query); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Query prepared above.
...
function upgrade_orgseries_grouping_from_one_five()
{
    ...
    $_old_groups = get_terms(array_merge($args_b, array('taxonomy' => 'category'))); //need to do this in order to get the description field.
...
function orgseries_grouping_import_existing_series()
{
    ...
    if (!($is_imported = get_option('orgseries_grouping_import_completed'))) {
        $series = get_terms(array('taxonomy' => ppseries_get_series_slug(), 'hide_empty' => false, 'fields' => 'ids'));
...
function orgseries_grp_term_filter($args, $taxonomies)
{
    ...
    global $_GET;
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only screen selection or display filter; no request-driven mutation.
    if (isset($_GET['ser_grp']) && in_array(ppseries_get_series_slug(), $taxonomies)) {
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only screen selection or display filter; no request-driven mutation.
    ...
    ?>
            <tr valign="top">
                <th scope="row"><label for="kill_grouping_on_delete">
                    <?php esc_html_e('Series Categories', 'organize-series'); ?>
                    </label>
                </th>
                <td>
                    <label>
...
function orgseries_groups_styles()
{
    ...
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only screen selection or display filter; no request-driven mutation.
    if ('edit-tags.php' == $checkpage && isset($_GET['taxonomy']) && ppseries_get_series_slug() == $_GET['taxonomy'] && isset($_GET['action']) && 'edit' == $_GET['action']) {
        ...
        <div id="side-info-column" class="inner-sidebar">
            <div id="side-sortables" class="meta-box-sortables">
                <div id="categorydiv" class="postbox">
                    <div class="handlediv" title="<?php esc_attr_e('Click to toggle', 'organize-series'); ?>"><br /></div><h3 class='hndle'><span><?php esc_html_e('Series Categories', 'organize-series'); ?></span></h3>
                    <div class="inside">
...
function edit_orgseries_group_fields($series, $taxonomy)
{
    ...
    <tr class="form-field term-groups-wrap">
            <th scope="row"><label for="groups"><h3 class='hndle'><span><?php esc_html_e('Series Categories', 'organize-series'); ?></span></h3></label></th>
            <td>
...
function orgseries_group_inline_edit($column_name, $type, $taxonomy)
{
    ...
    if ($taxonomy == ppseries_get_series_slug() && $column_name == 'group') { //takes care of WP3.1 changes (prevents duplicate output)
        ...
        <fieldset class="inline-edit-col-right inline-edit-categories"><div class="inline-edit-col">
            <span class="title inline-edit-categories-label"><?php esc_html_e('Series Categories', 'organize-series'); ?>
            </span>
...
// phpcs:disable WordPress.Security.NonceVerification.Missing -- WordPress verifies term create/edit requests before these taxonomy hooks.
function wp_insert_series_group($series_id, $taxonomy_id)
{
    global $_POST;
    $tax_input = isset($_POST['tax_input']) && is_array($_POST['tax_input']) ? map_deep(wp_unslash($_POST['tax_input']), 'absint') : array();
...
function orgseries_get_seriesid_from_group($group_id)
{
    ...
    if (!$grouppost || $grouppost->post_type != 'series_grouping') {
        return false;
    }
