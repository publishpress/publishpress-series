...
function series_get_icons($series)
{
    ...
    $row = $wpdb->get_row($wpdb->prepare("SELECT icon FROM " . esc_sql($tablename) . " WHERE term_id=%d", $series));
...
function seriesicons_write($series, $icon)
{
    ...
    if ($wpdb->get_var($wpdb->prepare("SELECT term_id FROM `" . esc_sql($tablename) . "` WHERE term_id=%d", $series))) {
