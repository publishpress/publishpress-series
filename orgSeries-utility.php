...
{
    if (!isset($posts)) {
        return false; //don't have the posts array so can't do anything.
    }
...
function _os_update_post_term_count($terms, $taxonomy)
{
    ...
    if ($object_types) {
        $object_types = array_values(array_filter(array_map('sanitize_key', $object_types), 'post_type_exists'));
    }
    ...
    foreach ((array) $terms as $term) {
        ...
        if ($object_types) {
            ...
            $query_args = array_merge($object_types, array( $term ));
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- IN placeholders are generated from registered post types above.
            $count += (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->term_relationships, $wpdb->posts WHERE $wpdb->posts.ID = $wpdb->term_relationships.object_id AND post_status IN ( 'publish', 'private' ) AND post_type IN ($object_type_placeholders) AND term_taxonomy_id = %d", $query_args));
        }
...
function is_ppseries_admin_pages()
{
    ...
    if (
        ( ( 'edit-tags.php' == $pagenow || 'term.php' == $pagenow  ) && ( isset($_GET['taxonomy']) && ppseries_get_series_slug() == sanitize_key(wp_unslash($_GET['taxonomy'])))  ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only admin screen detection.
        ( isset($_GET['page']) && in_array(sanitize_key(wp_unslash($_GET['page'])), $admin_pages) ) // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only admin screen detection.
    ) {
        return true;
    }
...
function ppseries_admin_settings_tabs()
{
    ...
    $settings_tabs = [
        'series_post_list_box_settings' => __('Post List Boxes', 'organize-series'),
        'series_post_details_settings'  => __('Post Details', 'organize-series'),
        'series_navigation_settings'    => __('Post Navigation', 'organize-series'),
    ];
