...
function _os_update_post_term_count($terms, $taxonomy)
{
    ...
    foreach ((array) $terms as $term) {
        ...
        if ($object_types) {
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- The IN fragment contains only generated string placeholders.
            $count += (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->term_relationships, $wpdb->posts WHERE $wpdb->posts.ID = $wpdb->term_relationships.object_id AND post_status IN ( 'publish', 'private' ) AND post_type IN ($type_placeholders) AND term_taxonomy_id = %d", array_merge($object_types, array($term))));
        }
