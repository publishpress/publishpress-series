...
if ( ! empty($series_ID) ) {
	...
	$form = '<form enctype="multipart/form-data" id="editseries" name="editseries" method="post" action="' . get_settings('siteurl') . '/wp-content/plugins/orgSeries/orgSeries-manage.php">'; //TODO: it is possible that the action will not point to the right location for orgSeries-manage.php file - check this if it doesn't work properly.
	$action = 'editedseries';
	...
	$form = '<form enctype="multipart/form-data" id="addseries" name="addseries" method="post" action="' . get_settings('siteurl') . '/wp-content/plugins/orgSeries/orgSeries-manage.php">'; //TODO see line 5
	$action = 'addseries';
