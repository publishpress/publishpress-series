...
function publishpress_series_process_upgrade()
{
    ...
    if (
        ...
        && current_user_can('manage_publishpress_series')
            ...
            $query = $wpdb->prepare("SELECT p.ID, t.term_id, pm.meta_value FROM $wpdb->posts AS p LEFT JOIN $wpdb->postmeta AS pm ON p.ID = pm.post_id LEFT JOIN $wpdb->term_relationships AS tr ON p.ID = tr.object_id LEFT JOIN $wpdb->term_taxonomy AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id LEFT JOIN $wpdb->terms AS t ON tt.term_id = t.term_id WHERE pm.meta_key = %s AND tt.taxonomy = %s", SERIES_PART_KEY, ppseries_get_series_slug());
            $posts = $wpdb->get_results($query); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Query prepared above.
