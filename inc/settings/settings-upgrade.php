...
{
    $series_action = isset($_REQUEST['series_action']) ? sanitize_key(wp_unslash($_REQUEST['series_action'])) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only notice suppression; upgrade actions verify nonce before processing.
    if (in_array($series_action, ['multiple-series-support', 'sync-menu-order'], true)) {
...
function publishpress_series_process_upgrade()
{
    global $wpdb;
    if (
        isset($_REQUEST['series_action'])
        && isset($_REQUEST['nonce'])
        && $_REQUEST['series_action'] === 'multiple-series-support'
            && wp_verify_nonce(sanitize_key(wp_unslash($_REQUEST['nonce'])), 'multiple-series-support-upgrade')
        && current_user_can('manage_publishpress_series')
    ) {
            //get list of posts that contain the meta key SERIES_PART_KEY
                $query = $wpdb->prepare(
                    "SELECT p.ID, t.term_id, pm.meta_value FROM $wpdb->posts AS p LEFT JOIN $wpdb->postmeta AS pm ON p.ID = pm.post_id LEFT JOIN $wpdb->term_relationships AS tr ON p.ID = tr.object_id LEFT JOIN $wpdb->term_taxonomy AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id LEFT JOIN $wpdb->terms AS t ON tt.term_id = t.term_id WHERE pm.meta_key = %s AND tt.taxonomy = %s",
                    SERIES_PART_KEY,
                    ppseries_get_series_slug()
                );
                $posts = $wpdb->get_results($query); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Prepared immediately above.
...
function publishpress_series_sync_menu_order()
{
    ...
    if (
        ...
        && $_REQUEST['series_action'] === 'sync-menu-order'
            && wp_verify_nonce(sanitize_key(wp_unslash($_REQUEST['nonce'])), 'sync-menu-order-action')
        && current_user_can('manage_publishpress_series')
    ) {
        $updated_count = 0;
